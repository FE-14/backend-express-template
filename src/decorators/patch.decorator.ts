...
export const Patch = (properties: { path: string, tag: string, isIndependentRoute?: boolean }, payload: Payload, middlewares: any[] = []): MethodDecorator => {
  ...
  return (target, propertyKey: string): void => {
    ...
    const apiDoc = GenerateApiDoc({ ...properties, ...{ method: "patch" } }, [], payload);
    routes.push({
      requestMethod: "patch",
